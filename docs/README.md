...
- **Trade-off:** ~200 lines of EdDSA we'd otherwise import. The risk is bounded: the only spec-bound piece is the in-circuit equation `S·B8 == R8 + 8·hm·A`; deterministic-nonce derivation only affects determinism, not validity. The `circomlib_circuit_equation_holds` unit test asserts that exact equation in pure Rust.

### Deterministic account seeding (1000 accounts at startup)
- **Decision:** a one-shot `seed_accounts` container generates 1000 accounts deterministically from index `0..999`, inserts them, computes the genesis Poseidon root, and prints `GENESIS_ROOT=0x…` for `run-fresh.sh` to pass to the `Deploy` script as `INITIAL_STATE_ROOT`. Idempotent (re-running against a populated DB is a no-op).
- **Why:** the genesis state is a fixed artifact - capturing it once at deploy time is simpler than reconciling runtime registrations against a moving L1 root, and makes "rebuild from scratch" reproducible.
- **Trade-off:** adding an account post-genesis needs a contract-level registration (not implemented) or a wipe + `run-fresh.sh`. Key derivation per account: see "L2 Key Derivation" above.

### Test-client traffic (20 signed txs/cycle)
The test client runs continuously: it refreshes nonces for all accounts from the sequencer's `/accounts` endpoint, picks 20 random distinct (sender, receiver) pairs, signs each with real EdDSA (`accounts::sign_tx`), POSTs them, then sleeps a random 10-30 s. 20 = the batch size, so each cycle produces roughly one full batch - realistic continuous traffic without overwhelming the prover.

### Non-custodial L2 keys (client-side)
L2 keys live with the wallet (here, `tools/accounts`), never with the sequencer. A sequencer that held user keys would be custodial - able to forge transactions, the same trust model as a centralized exchange. (Derivation details: see "L2 Key Derivation" above.)

### Data availability = Ethereum calldata
Each batch's transactions are posted to L1 as **calldata** (the most secure, most expensive option) and committed via `batchDataHash`, so anyone can reconstruct L2 state from L1 alone. Cheaper channels - EIP-4844 blobs (pruned later) or external DA layers (Celestia / EigenDA, weaker security) - are future work.

## Limitations (research prototype)

Deliberate scope choices for a laptop-scale research build - not bugs. Full tracking in
[`../_simulation-logs/TODO_security.md`](../_simulation-logs/TODO_security.md) (research
correctness) and [`./ToDo-for-industry-deployments.md`](./ToDo-for-industry-deployments.md)
(production hardening).

- **Single, centralised sequencer:** a single point of failure for liveness and censorship.
- **Fixed batch size = 20:** chosen so witness generation fits in 16 GB RAM; production amortises L1 cost over much larger batches.
- **Single ERC-20 asset, no fees:** one bridged token; the `fee` field exists but is never charged. Not ETH, and there is no native coin.
- **Bridge not wired end-to-end:** `deposit()` queues on L1 but isn't credited on L2; withdrawals aren't claimable; `withdrawalsRoot` is operator-attested, not proof-bound.
- **Index-keyed tree:** a fixed-depth Poseidon Merkle tree keyed by `leaf_index`, not a true address-keyed sparse Merkle tree.
- **Single-contributor Phase-2 trusted setup:** sound only if that one contributor was honest; production needs a multi-party ceremony.
...
| Operation | Hash | Why |
| :--- | :--- | :--- |
| L2 state Merkle tree (internal nodes) | Poseidon | ZK-friendly, cheap inside the circuit |
| L2 account leaf | Poseidon(address, balance, nonce) | Matches circuit's leaf encoding |
| L2 address derivation | Poseidon(pubkey_x, pubkey_y) | Two-stage pattern, stable identifier |
| L1 batch commitment | Keccak-256 | Ethereum-native, cheap on L1 |
| L1 verifyProof gas | Auto-generated by snarkjs | Pairing-based, very small |

### Tooling dependencies

| Tool | Why |
| :--- | :--- |
| Foundry (forge / cast / anvil) | L1 contract compile, test, local devnet |
| Docker (or Colima) | Container orchestration for all L2 services |
| Rust 1.91+ | Builds all off-chain crates |
| Node + snarkjs | Witness generation + Groth16 proving |
| circomlib | EdDSA, Poseidon, Mux templates used by the circuit |

### Colima settings (lightweight Docker on macOS)

If using Colima, recommended starting values:
- CPU: 4
- Memory: 8 GB
- Runtime: docker
...
```
.
├── circuits/                # Circom source + build artifacts
├── docs/                    # This file and supporting docs
├── offchain/crates/         # Rust services (sequencer, prover, submitter, archiver)
│   ├── types/               # Shared types (L2Transaction, Batch, Poseidon helpers, keys)
│   ├── merkle/              # Fixed-depth Poseidon tree + Keccak Merkle tree
│   ├── execution/           # State transition function
│   └── storage/             # SQL queries grouped by service
├── tools/test_client/       # CLI client for sending test transactions
├── src/                     # Solidity sources (Rollup, BridgeERC20, Verifier)
├── script/                  # Foundry deployment / interaction scripts
├── test/                    # Foundry tests
├── docker-compose.yml       # Service orchestration
├── init.sql                 # PostgreSQL schema
└── .env                     # Local secrets (L1_PRIVATE_KEY, addresses)
```

### Security & protocol discussion topics

> Discussion points - not all implemented. Tracked work lives in
> [`_simulation-logs/TODO_security.md`](../_simulation-logs/TODO_security.md) (research correctness)
> and [`docs/ToDo-for-industry-deployments.md`](./ToDo-for-industry-deployments.md) (production hardening).

- **Censorship:** single sequencer vs. decentralised set; forced inclusion escape hatch.
- **DA:** calldata vs. blobs; fraud risks if DA is off-chain.
- **Upgradability:** verifying key rotation; circuit versioning; governance guardrails.
- **Soundness:** trusted setup vs. universal setup; toxicity of CRS; mitigation via MPC ceremonies.
- **Liveness:** what happens if sequencer stalls? exit mechanisms.
- **Fees & MEV:** ordering policy and fairness.
- **Bridging risks:** replay protection, double-mint prevention via nonces.
